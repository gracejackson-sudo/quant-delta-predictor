...
\texttt{INSUFFICIENT CALIBRATION} and no interval.

The refusal is scored on \emph{one-sided} coverage: how often the true delta
stayed at or above the interval's lower bound. The risk this tool bounds is
accuracy loss, and a model that beats its envelope has exposed no one to
anything, so counting that as a failure would refuse cells that are merely
outperforming. Two cells are refused: \texttt{w4a16|<2B}, where losses stayed
above the lower bound \OneSidedCellWfourSmall\% of the time over
\RowsCellWfourSmall{} rows from \CkptsCellWfourSmall{} checkpoints
(two-sided containment \CovCellWfourSmall\%), and \texttt{w8a16|>10B} at
\OneSidedCellWsixteenBig\% over \RowsCellWsixteenBig{} rows from
\CkptsCellWsixteenBig{} checkpoints (two-sided \CovCellWsixteenBig\%).
The distinction was identified by an external reader. Comparing a one-sided
measurement against the printed 90\% is the \emph{lenient} reading, not a
flattering one: the interval is symmetric, so the permitted 10\% is split
across two tails, and pooled one-sided coverage across all scored rows is
\PooledOneSided\%. Both cells are refused even under the lenient comparison.
