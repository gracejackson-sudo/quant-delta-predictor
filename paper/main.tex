...
best predictor, a per-scheme mean, beats a global mean by \(\MaeGain\)pp of MAE, and both ridge
...
of paired measurements of exactly the quantity of interest (\Nrows{} in our harvest).
...
\begin{enumerate}
  ...
  \item \textbf{A calibrated envelope that does work} (\S\ref{sec:results}),
    ...
    and an explicit rule that withholds an interval where coverage is poor or the evidence is too thin to judge.
...
\emph{insufficient evidence}, again with no interval, when the cell rests on
...
that case the evidence cannot support either of the other two verdicts.
...
Two cells were flagged under the earlier two-state rule, and neither is a
...
support floor, and note that a cluster bootstrap over model configurations does not
...
\(\ProspCov\%\) coverage on unseen checkpoints, that declines to answer where its own
measured coverage is poor or too thinly supported to judge. The more useful contribution may be the limitation
