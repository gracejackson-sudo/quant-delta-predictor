...
split-conformal intervals with \(\ProspCov\%\) empirical coverage
...
\paragraph{Split conformal.} Following split conformal regression~\cite{lei2018distribution}, given calibration residuals
...
coverage is measurably poor on a sufficient number of scored rows. It is
...
across two tails, and pooled one-sided coverage across all scored rows is
\PooledOneSided\%.
...
checkpoints (mean gap \CtrlGap{}pp, \(z = \CtrlZmin\) and \(\CtrlZmax\)). We
attribute this to approximately \CalibRatio\(\times\) less calibration data than
production defaults; we found no bug. Consequently, excess-over-control measures
...
but the unit of analysis remains a row.
...
\(-68\)pp delta) and the 90\% marginal-interval figure in \S\ref{sec:method},
which are quoted from the provenance record rather than recomputed.
