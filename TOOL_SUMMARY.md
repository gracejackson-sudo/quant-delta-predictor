...
No network, no API key, no GPU; three common packages:
...
The second command is the one worth running: it is a cell the tool **refuses**, and it prints why rather than returning a number.
...
Two (scheme, size) cells have measured coverage far below nominal. For these the tool prints `INSUFFICIENT CALIBRATION` and no interval:
...
A further 4<!-- claim: n_cells_blocked_from_tier_a = 4.0000 --> cells cannot reach Tier A regardless of coverage, because they rest on fewer than 3<!-- claim: min_cell_checkpoints = 3.0000 --> distinct checkpoints. `fp8|<2B` is the clearest case: 100.0<!-- claim: cell_coverage_pct::fp8|<2B = 100.0000 -->% measured coverage from 1<!-- claim: cell_train_ckpt2::fp8|<2B = 1.0000 --> checkpoint.
