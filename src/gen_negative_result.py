...
def main():
    ...
    A(f"Re-running MMLU under the published protocol (5-shot, letter-scored) "
      ...
      f"Qwen2.5-0.5B-Instruct scores {n('s2_base_qwen05','{:.2f}')} "
      f"against the Instruct card's {n('s2_pub_qwen05','{:.2f}')} "
      f"(the base-model card gives {n('s2_pub_qwen05_base','{:.2f}')}), and "
      f"our Qwen2.5-1.5B-Instruct scores "
      f"{n('s2_base_qwen15','{:.2f}')}. The harness "
      f"confound is closed.\n")
    A(f"Closing it exposed a larger one. Our control ran the *Instruct* "
      f"checkpoints, but Red Hat has not published W4A16 for those; the "
      f"nearest available comparators are the *base-model* W4A16 cards. So "
      f"the comparison is Instruct-vs-base, not like-for-like: our "
      f"*correctly-configured* Instruct control loses "
      f"{n('ctrl_our_delta::Qwen2_5-0_5B','{:+.2f}')}pp and "
      f"{n('ctrl_our_delta::Qwen2_5-1_5B','{:+.2f}')}pp against the "
      f"base-model W4A16 losses of "
      f"{n('ctrl_pub_delta::Qwen2_5-0_5B','{:+.2f}')}pp and "
      f"{n('ctrl_pub_delta::Qwen2_5-1_5B','{:+.2f}')}pp - "
      f"**{n('ctrl_ratio_x','{:.1f}')}x more damaging** (z = "
      f"{n('ctrl_min_z','{:.1f}')} and {n('ctrl_max_z','{:.1f}')}). Instruct "
      f"models may be more fragile than their base counterparts under 4-bit, "
      f"so the ratio is indicative rather than a clean estimate; a "
      f"matched Instruct-vs-Instruct control is deferred future work. The "
      f"pattern is consistent with roughly {n('calib_ratio_x','{:.0f}')}x "
      f"less calibration data than production defaults, though we have not "
      f"independently tested the attribution.\n")
    A("So the excess-over-control figures measure **bad recipe against "
      "mediocre recipe**, not against production. That makes them an "
      "indicative anchor rather than a bound, and it is why no corrected "
