...
# ---------------------------------------------------------- scoring
@torch.no_grad()
def score_letters(model, tok, items, by_sub, bs=8):
    ...
    for i in range(0, len(items), bs):
        ...
        lg = model(input_ids=inp.to(DEV),
                   attention_mask=att.to(DEV)).logits[:, -1, :].float()
        lp = lg.log_softmax(-1)[:, ids_letter]
...
def main():
    ...
    ar, ar_shots = arc_items()
    hl, hl_shots = hella_items()
    ar_prefix = fewshot_prefix(ar_shots)
    hl_prefix = fewshot_prefix(hl_shots)
    log(f"[data] mmlu {len(mm)} (5-shot), arc {len(ar)} (25-shot), "
        f"hellaswag {len(hl)} (10-shot)")

    def score_all(model, tok):
        return {"mmlu": score_letters(model, tok, mm, by_sub),
                "arc_challenge": score_cont(model, tok, ar, ar_prefix),
                "hellaswag": score_cont(model, tok, hl, hl_prefix)}
    ...
    for mid, pb in MODELS:
        ...
        for tag, scheme, bits, group, pt, ck, method in RECIPES:
            ...
            json.dump(out, open(f"{OUTDIR}/fiveshot_runs.json", "w"), indent=1)
        base.load_state_dict(sd)
        del base
        torch.cuda.empty_cache()

    if out:
        with open(f"{OUTDIR}/fiveshot_runs.csv", "w", newline="") as f:
